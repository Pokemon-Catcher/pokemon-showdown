...
export const Items: ModdedItemDataTable = {
	eviolite: {
		...
		onModifyDef: function (def, pokemon) {
			let name = pokemon.baseSpecies.baseSpecies;
			let template2 = pokemon.baseSpecies;
			if (pokemon.name !== undefined) {
				name = pokemon.name.substring(1);
				template2 = Dex.species.get(name);
			}
			if (!template2.exists) {
				name = pokemon.baseSpecies.baseSpecies;
	...
	stick: {
		...
		onModifyCritRatio: function (critRatio, user) {
			let  name = user.baseSpecies.baseSpecies;
			let template2 = user.baseSpecies;
			if (user.name !== undefined) {
				name = user.name.substring(1);
				template2 = Dex.species.get(name);
			}
			if (!template2.exists) {
				 name = user.baseSpecies.baseSpecies;
				template2 = user.baseSpecies;
			}
			if (
